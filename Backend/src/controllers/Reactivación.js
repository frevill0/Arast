...
export const ConsultaSuspension = async (req, res) => {
    ...
    try {
      ...
      const estadoSuspencion = 'Suspención Temporal';
      if (!estadoSuspencion == encontrarSocio.Estatus) {
        console.log("Estatus:", encontrarSocio.Estatus);
        return res.status(404).send({ msg: "El socio no se encuentra en estado de suspensión" });
      }
